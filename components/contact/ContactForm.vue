...
<template>
    <div class="contact-form">
        <div class="contact-form__header">
            <button :class="{'contact-form__button--active':formToDisplay == 'contact'}" class="contact-form__button h5" @click="setFormToDisplay('contact')">Contact us</button>
            <button :class="{'contact-form__button--active':formToDisplay == 'job'}"  class="contact-form__button h5" @click="setFormToDisplay('job')">Join our team</button>
        </div>
        <form name="contact" @submit.prevent="submitForm" class="contact-form__form" :class="{'contact-form__form--displayed': formToDisplay == 'contact'}" action="" netlify>
        ...
        <form name="jobs" @submit.prevent="submitForm"  class="contact-form__form" :class="{'contact-form__form--displayed': formToDisplay == 'job'}" action="" netlify>
                <contact-input name="email" type="email" label="Email" required/>
                <contact-input name="subject" label="Subject" required/>
                <contact-textarea name="message" label="Message" required/>
